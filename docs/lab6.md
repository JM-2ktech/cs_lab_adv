...
**어디쯤** — 사람이 승인한 데이터에 **AI의 판단을 기록**하는 단계입니다.
...
Lab 3에서 에이전트가 적합도를 **말로 답하게** 만들었습니다. 이 랩에서는 그 값을 **목록에 기록합니다.**
...
| 게이트 | 흐름의 조건 노드 | 통과하지 못하면 **거기서 멈추는** 관문. 이 랩엔 두 겹이 있다 |
...
8. 안쪽 **True 분기**에 SharePoint **항목 업데이트**를 추가합니다. **사이트·목록** 지정 후 **ID** 칸에 식 모드로 입력합니다:
    ...
    {: .note }
    **이 식이 하는 일** — 조회 결과 목록에서 `first()`로 **첫 번째 한 건**을 가져와 그 `ID`를 읽습니다. 앞의 게이트가 "정확히 1명"을 이미 보장했으므로, 첫 번째 = 유일한 그 사람입니다. `?['...']`는 값이 없어도 오류 없이 넘어가라는 안전 표기입니다.
