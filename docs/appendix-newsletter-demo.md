...
오늘 만든 것은 **HR 채용**입니다. 같은 구조를 다른 업무에도 사용합니다.
...
둘 다 **오늘 이미 만들어 본 것**입니다. 시연 시간을 줄이려고 이번에는 뺐습니다.
...
{: .note }
**한 군데를 일부러 다르게 했습니다.** Logic Apps 문서는 RSS를 **트리거**로 씁니다 — 새 기사가 뜰 때마다 흐름이 돌아 **기사 1건당 메일 1통**이 갑니다. 이 시연은 **일정 트리거 + RSS 액션**이라, 주기마다 한 번 돌면서 그 사이 기사를 **한 통에 묶어** 보냅니다. 뉴스레터에는 후자가 맞습니다. 같은 부품이라도 트리거로 등록할 때와 액션으로 등록할 때의 결과가 다릅니다.
