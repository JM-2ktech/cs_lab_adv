...
<!-- 저작 메모(학생 비노출):
     ...
     - ★ 수강생에게 Approve(SP 콘텐츠 승인) 권한은 필요 없다. 2026-07 고객사 실측 결과 SP 표준 승인을 쓰려면
...
Lab 3의 지침에 적은 **"승인된 지원자만 보여준다"** 는 이 랩부터 동작합니다. 시드 지원자가 전부 승인 상태여서 지금까지는 차이가 드러나지 않았습니다.
...
10. **사이트 주소**·**목록 이름**은 2번과 똑같이 지정하고, **ID** 칸에 트리거의 **ID** 칩을 넣습니다.
    ...
    ![사이트·목록·ID + 요약승인상태 Value = 승인됨](../assets/lab5/10.png)
...
12. 사이트 주소·목록 이름·ID는 True 분기와 똑같이 지정합니다. `요약승인상태 Value`만 `거부됨`으로 바꿉니다.
    ...
    ![사이트·목록·ID + 요약승인상태 Value = 거부됨](../assets/lab5/12.png)
