---
title: 중급 Lab 4. 적재 흐름 (Outlook 메일 트리거)
nav_order: 5
nav_exclude: true
search_exclude: true
---
...
<!-- 저작 메모(학생 비노출):
     ★ 2026-09-07 비활성화. 차기 고객사가 Outlook 미사용이라 lab4-alt 를 Lab 4 본편으로 올렸다.
       nav_exclude·search_exclude 만 걸었다 — 파일과 URL 은 살아 있어 강사는 직접 주소로 연다.
       되살릴 때: 그 두 줄을 지우고 title 에서 "(Outlook 메일 트리거)" 를 뗀다.
       lab4-alt 는 nav_order 6 · title "중급 Lab 4-대안. 적재 흐름 (Outlook 미사용)" 으로 되돌린다.
     ...
     - 사이트 참조 = 값 직접 입력(https://2ktech.sharepoint.com/sites/edulab). ResumeLink = concat('<site>', 파일만들기 Path).
       2026-09-07 본문의 환경 변수 언급 제거 — 사유는 lab1.md 메모. 전 랩 공통.
...
**왜** — 오전 내내 다룬 지원자 데이터가 **어떻게 목록에 들어왔는지**를 이제 직접 만듭니다. 이게 없으면 사람이 이력서를 하나씩 읽고 손으로 입력해야 합니다.

**어디쯤** — 파이프라인의 **맨 앞**입니다. 여기서 만들어진 항목이 Lab 5~7의 재료가 됩니다.

```mermaid
flowchart LR
    M["📧 이력서 메일"] --> L4["적재 흐름<br/>Lab 4 ★"]
    L4 --> L5["승인<br/>Lab 5"]
    L5 --> L6["적합도<br/>Lab 6"]
    L6 --> L7["전형단계<br/>Lab 7"]
...
| 용어 | 화면에서 | 한 줄 |
|---|---|---|
| 트리거 | 흐름 맨 위 첫 상자 | 흐름을 **깨우는** 사건. 여기서는 "메일이 도착하면" |
...
| For each | 자동으로 생기는 반복 상자 | 첨부파일이 여러 개일 수 있어 **하나씩 돌리는** 구조 |
...
테스트용 **신규 지원자 이력서 PDF**(시드 10명과 다른 인물)를 내려받아 둡니다.
...
12. **프롬프트** 드롭다운에서 **새 사용자 지정 프롬프트**를 선택합니다. 이후 지침에 아래 내용을 넣습니다. (이 프롬프트의 내용은 아래 참조)
    ...
    **제공 프롬프트 내용 (참조)** — 비정형 이력서 → 정형 필드로 변환하는 추출기입니다.
    ...
    ```
    경력사항·지원직군이 **고정된 선택지로만 출력**되므로, 뒤에서 선택형 컬럼(CareerLevel)에 그대로 매핑해도 값이 일치합니다. 출력 형식이 [JSON](./glossary.html#term-json)인 이유도 같습니다 — 사람이 읽는 문장이 아니라 **다음 단계가 칸별로 꺼내 쓸 수 있는 모양**이어야 하기 때문입니다. 모델은 **GPT-4.1 mini**.
...
13. 지침 아래쪽에 커서를 위치하고 `이력서 원본 :` 을 타이핑합니다. 이어서 `/`를 입력하면 나타나는 목록에서 **이미지 또는 문서**를 선택합니다.
    ...
    **문서 입력** 패널에서 **이름**을 `Resume`로 지정합니다. **샘플 데이터**에 [이력서 샘플](../assets/download/추가이력서샘플/박도윤.pdf)을 업로드해 두면 다음 단계에서 바로 테스트해 볼 수 있습니다.
    ...
    <!-- 저작 메모(학생 비노출):
         ...
         신규 13b(/ 드롭다운 · 이미지 또는 문서)·13c(문서 입력 이름 Resume + 샘플 데이터 업로드) 촬영분 삽입.
         이로써 "이력서 샘플 업로드 안내 필요" 건(실습 중 발견) 해소 — 업로드(13c) → 테스트·JSON 확인(13d)로 닫힘.
         이미지 노출 순서는 화면 진행 순(13b → 13c → 13 → 13d). 13.png는 칩이 삽입된 '결과' 컷이라 뒤에 둠.
         ※ 같은 블록이 lab4-alt.md 8번에도 있음 — 한쪽 고치면 반드시 같이 볼 것.
...
17. 폴더 경로는 `DocLib` > `이력서샘플` 을 지정합니다.
...
23. **제목(Title)** 칸의 **fx**를 열고 아래를 입력합니다. `지원자명`·`지원직군`은 식 안에서 **동적 콘텐츠 칩**으로 삽입합니다(키보드 텍스트 아님). `지원자명`, `지원직군`은 `프롬프트 실행` 결과로 나온 값입니다. 칩을 추가하면 인코딩 처리가된 테스트가 나오므로 이하 fx함수의 정확한 위치에 잘 배치해야합니다.
