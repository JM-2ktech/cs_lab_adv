...
**왜** — 이전 랩에서 처리한 지원자 데이터가 **어떻게 목록에 들어왔는지**를 직접 만듭니다. 이 흐름이 없으면 사람이 이력서를 하나씩 읽고 직접 입력해야 합니다.
...
Lab 3까지는 이미 있는 데이터를 **읽기만** 했습니다. 이 랩부터는 데이터를 만듭니다 — [읽기는 커넥터+지침, 상태 변경은 흐름](./before-you-start.html)입니다.
...
| For each | 자동으로 생기는 반복 상자 | 첨부파일이 여러 개일 수 있어 **한 건씩 처리하는** 구조 |
...
테스트용 **신규 지원자 이력서 PDF**(시드 10명과 다른 인물)를 준비해 둡니다.
...
12. **프롬프트** 드롭다운에서 **새 사용자 지정 프롬프트**를 선택합니다. 이후 지침에 아래 내용을 넣습니다. (이 프롬프트의 내용은 아래 참조)
    ...
    **제공 프롬프트 내용 (참조)** — 비정형 이력서 → 정형 필드로 변환하는 추출기입니다.
    ...
    ```
    경력사항·지원직군이 **고정된 선택지로만 출력**되므로, 뒤에서 선택형 컬럼(CareerLevel)에 그대로 매핑해도 값이 일치합니다. 출력 형식이 [JSON](./glossary.html#term-json)인 이유도 같습니다 — 사람이 읽는 문장이 아니라 **다음 단계가 칸별로 참조할 수 있는 형식**이어야 하기 때문입니다. 모델은 **GPT-4.1 mini**.
