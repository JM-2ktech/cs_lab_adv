...
<!-- 저작 메모(학생 비노출):
     ★ 2026-09-07 이 문서가 Lab 4 본편이다. 차기 고객사가 Outlook 미사용이라 lab4.md 를 nav 에서 감췄다.
       종전 title "중급 Lab 4-대안. 적재 흐름 (Outlook 미사용)" · nav_order 6. 되돌릴 때 이 값으로.
       본문에 "원본 Lab 4"·"대안" 표현이 남아 있다(For each 설명·이메일 필드 비교·마무리 콜아웃).
       숨긴 랩을 가리키는 말이라 전환이 확정되면 같이 손본다.
...
{: .important }
**이력서 파일을 흐름에 직접 올려 적재합니다.** 메일함을 거치지 않으므로 Outlook 계정이 없어도 됩니다. 완성물은 뒤따르는 랩이 기대하는 것과 같습니다 — **본인 지원자 목록에 항목 1건**.
...
- 테스트용 **신규 지원자 이력서 PDF**(강사 제공, 시드 10명과 다른 인물)를 내려받아 둡니다. 
- 흐름 실행 시 직접 입력할 원본 Lab 4는 메일 발신자에서 자동으로 가져오지만, 이 대안은 메일이 없으므로 직접 입력합니다.
