...
<!-- 저작 메모(학생 비노출):
     - 근거: 제작자가 Copilot Studio에서 직접 만든 것을 리버스로 문서화(2026-07-01). 개념검증 완료 확인.
     - 도입 시점: Lab 3(에이전트 구성) 완료 직후가 자연스러운 진입점 — 이때 처음으로 "내 면접관 에이전트가 잘 만들어졌나?"를 확인할 대상이 생김. 단, 고정된 시점은 아니고 Lab 7까지 계속 쓰는 동반 도구.
     - 하위 에이전트(Connected/Child agent) = 실습생 본인의 면접관 에이전트. 제작자 개인환경에서는 "면접관 에이전트 교육"이라는 이름으로 붙였는데, 이건 제작자의 테스트용 사본 이름일 뿐 — 학생 안내 시 "본인이 만든 에이전트"를 선택하도록 명시할 것 (아래 6단계에 반영).
     - Knowledge = 강의 사이트 GitHub Pages(https://jm-2ktech.github.io/cs_lab_adv/) 웹 소스로 추가.
     ...
     2026-09-08: Knowledge URL 을 구본 cs_lab_v2 → 중급 정본 jm-2ktech.github.io/cs_lab_adv 로 바꿨다.
                 구본 사이트는 아직 살아 있으나 2026-08-27 폐기본이라 랩 지침이 낡았다.
                 이 에이전트가 그것을 「정답지」로 삼으면 낡은 기준으로 판정한다.
     - 이름 확정: "자가진단 에이전트" (제작자 결정, 2026-07-01. 후보였던 사수/코치/닥터/QA는 기각).
...
{: .important }
이 에이전트는 **Lab 3(에이전트 구성)를 마친 뒤부터** 만들 수 있습니다. 진단할 대상(면접관 에이전트)이 있어야 하기 때문입니다. 한 번 만들어 두면 **Lab 7까지 계속 사용합니다** — 랩을 하나씩 마칠 때마다 다시 불러 방금 만든 것이 정상 동작하는지 점검합니다.
...
3. **Knowledge**에 강의 사이트를 추가합니다. 웹사이트 소스로 아래 URL을 입력하고, **이름**과 **설명**도 함께 채웁니다.
    ...
    - **URL**: `https://jm-2ktech.github.io/cs_lab_adv/`
    - **이름**: `실습 가이드 사이트`
    - **설명**:
      ```
...
- [ ] Knowledge에 강의 사이트(`https://jm-2ktech.github.io/cs_lab_adv/`)가 추가됐다
