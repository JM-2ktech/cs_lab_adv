...
> **이번 랩 완성물**: 지원자 10명과 이력서 파일이 들어 있는 **내 SharePoint 목록**
>
> **예상 시간**: 10분 
...
<!-- 저작 메모(학생 비노출):
     ...
     ★ 흐름 1개 = "SP List 사전 데이터 복제" (리스트 생성 + 데이터 10명 적재 + PDF 적재 통합).
     ★ 수강생은 해당 흐름을 "다른 이름으로 저장" → "SP List 사전 데이터 복제 (본인이름)" 으로 소유권 이전.

     ═══ 환경 프로비저닝 구조 (2026-07-31 정리) ═══
     흐름은 두 갈래다. 헷갈리지 말 것.
       [수강생용] "SP List 사전 데이터 복제" — Lab 1에서 각자 복사해 실행. 리스트 생성 + 지원자 10명 적재.
     ...
     ★ 사이트 주소는 전 랩에서 값 직접 입력(https://2ktech.sharepoint.com/sites/edulab).
       2026-09-07: 본문에서 환경 변수 언급을 전부 뺐다. 교육 환경에 환경 변수가 없고 SP 사이트를 사전 안내하므로
       "왜 안 쓰는가"를 설명할 대상 자체가 없다. 되살리지 말 것.
     ★ 실행 UI 라벨(테스트→수동→실행)은 촬영 시 최종 확인.
     ★ 2026-09-07: 환경 이름을 _config.yml 의 lab_env 로 뺐다. 현재 값 "교육"(에듀랩 실측 환경).
       본문에 직접 쓰지 않는다 — 회차마다 바뀌는 값이라 lab2.md 와 두 곳에서 어긋난 적이 있다. -->
...
1. 브라우저에서 [https://make.powerautomate.com](https://make.powerautomate.com/) 으로 이동합니다. 우측 상단 **환경 선택기**를 클릭해 **{{ site.lab_env }}** 환경을 선택합니다.
...
3. **`HR 채용 자동화 사용자 용`** 솔루션을 찾아 클릭해 엽니다.
...
8. 새 탭에서 교육용 **SharePoint 사이트**로 이동해 내 **지원자** 리스트를 엽니다.
    ...
    {: .important }
    이곳에 생성한 리스트 이름과. 유형 `목록`을 확인하시고 사전데이터 10인이 잘 보이는지 테스트해주세요.
